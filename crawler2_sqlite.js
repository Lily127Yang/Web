    ...
    request(seedURL, function(err, res, body) { //读取种子页面
        ...
        seedurl_news.each(function(i, e) { //遍历种子页面里所有的a链接
            ...
            try {
                ...
                if (href.toLowerCase().indexOf('http://') >= 0 || href.toLowerCase().indexOf('https://') >= 0) myURL = href; //http://开头的或者https://开头的
            ...
            sqlite.select(fetch_url_Sql, fetch_url_Sql_Params, function(res) {
                if (res == undefined) {
...
function newsGet(myURL) { //读取新闻页面
    request(myURL, function(err, res, body) { //读取新闻页面
        ...
        var fetchAddSql_Params = [fetch.url,
            fetch.title, fetch.content, fetch.publish_date,
        ];
